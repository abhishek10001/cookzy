...
//   return (
//     <section className="relative w-full py-20 px-4 bg-gradient-to-b from-white to-orange-50" id="speciality">
...
const SpecialityMenu = () => {
  ...
  return (
    <section className="relative w-full py-16 bg-gradient-to-b from-white to-orange-50/50" id="speciality">
      <div className="container mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-12">
          <h2 className="text-4xl font-extrabold text-gray-900 mb-4 tracking-tight">Find By Speciality</h2>
          <div className="w-24 h-1.5 bg-orange-500 mx-auto mb-6 rounded-full"></div>
          <p className="text-gray-600 text-xl max-w-3xl mx-auto leading-relaxed">
            Discover our curated selection of culinary experts. Browse through specialties and find the perfect chef for your next gastronomic adventure!
          </p>
        </div>
        ...
        <div className="relative w-full">
          ...
          {showControls && (
            <>
              <button 
                className={`absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 p-3 rounded-full shadow-xl text-orange-500 hover:bg-orange-500 hover:text-white transition-all duration-300 ${scrollPosition <= 5 ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}`}
                onClick={scrollLeft}
                disabled={scrollPosition <= 5}
              >
              ...
              <button 
                className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 p-3 rounded-full shadow-xl text-orange-500 hover:bg-orange-500 hover:text-white transition-all duration-300 ${scrollPosition >= maxScroll - 5 ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}`}
                onClick={scrollRight}
                disabled={scrollPosition >= maxScroll - 5}
              >
          ...
          <div 
            ref={scrollRef} 
            onScroll={handleScroll}
            className="w-full flex gap-6 pb-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory pl-4 pr-4"
            style={{ 
              scrollbarWidth: 'none', 
            ...
            {specialityData.map((item, index) => (
              ...
              >
                <Link 
                  ...
                  className="block"
                >
                  <div className={`relative group w-64 flex flex-col items-center transition-all duration-500 ${activeIndex === index ? 'scale-105' : ''}`}>
                    {/* Image Container */}
                    <div className="relative mb-5 rounded-2xl overflow-hidden shadow-lg transition-all duration-500 group-hover:shadow-2xl">
                      <div className="w-60 h-60 overflow-hidden rounded-2xl">
                        <img 
                          className="w-full h-full object-cover transition-all duration-500 group-hover:scale-110" 
                          src={item.image} 
                          alt={item.speciality} 
                      ...
                      {/* Overlay */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-2xl flex items-end justify-center">
                        <div className="text-white pb-8 font-semibold text-lg">View Chefs</div>
                      </div>
                    </div>
                    
                    {/* Label */}
                    <div className="relative">
                      <h3 className={`text-xl font-semibold text-center transition-all duration-300 ${activeIndex === index ? 'text-orange-600' : 'text-gray-800'}`}>
                        {item.speciality}
                      </h3>
                      <div className={`h-1 bg-orange-500 transition-all duration-300 ${activeIndex === index ? 'w-full' : 'w-0'} mx-auto mt-1.5 rounded-full`}></div>
                    </div>
                  </div>
                </Link>
              </div>
        ...
        <div className="md:hidden flex justify-center gap-2 mt-6">
          {specialityData.map((_, index) => (
            <div 
              key={index}
              className={`w-2 h-2 rounded-full transition-all ${index === Math.floor(scrollPosition / 260) ? 'bg-orange-500 w-6' : 'bg-gray-300'}`}
            />
          ))}
        </div>
        ...
        <div className="text-center mt-12">
          <Link 
            to="/cooks"
            onClick={() => window.scrollTo(0,0)} 
            className="inline-block px-10 py-4 bg-primary text-white rounded-full font-semibold text-lg hover:bg-orange-600 transition-all duration-300 transform hover:scale-105 shadow-md hover:shadow-xl"
          >
            View All Categories
          </Link>
        </div>
